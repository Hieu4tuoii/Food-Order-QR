...
public class OrderServiceImpl implements OrderService {
    ...
    @Transactional
    public Long saveOrderByAI(OrderByAIRequest request) {
        log.info("Request save order {}", request.toString());
        Customer customer = customerRepository.findById(request.getCustomerId())
                .orElseThrow(() -> new ResourceNotFoundException("Customer not found"));
        if(request.getDiningTableId()==null){
            throw new ResourceNotFoundException("Dining table id is null");
        }
        DiningTable diningTable = diningTableRepository.findById(request.getDiningTableId())
                .orElseThrow(() -> new ResourceNotFoundException("Dining table not found"));
        //lay hoa don gan nhat cua ban an
        ...
        //tao ds order mới
        for(OrderItemByAIRequest foodOrderByAIRequest : request.getItems()){
            //tim kiem mon an theo id
            Food food = foodRepository.findByIdAndStatusIn(foodOrderByAIRequest.getFood_id() , FoodStatus.getValidStatuses())
                    .orElseThrow(() -> new ResourceNotFoundException("Food not found"));
            //tao order detail mới
            OrderDetail orderDetail = OrderDetail.builder()
                    .quantity(foodOrderByAIRequest.getQuantity())
